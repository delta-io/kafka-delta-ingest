...
* Executing kafka-delta-ingest directly on WSL2/Ubuntu 20.04 isn't stable
** A Dockerfile is provided to run kafka-delta-ingest on Windows
*** Tested with Docker Desktop using the link:https://docs.docker.com/desktop/windows/wsl/[WSL2-based engine].


==== Starting Worker Processes

1. Create Docker image: `docker build -t kafka-delta-ingest:v0.1.0 . -f Dockerfile.Debian`
2. Run kafka-delta-ingest against the web_requests example topic and table (customize arguments as desired):

```bash
docker run -it --network=host \
  -e SSL_CERT_FILE=/etc/ssl/certs/ca-certificates.crt \
  -e AZURE_STORAGE_ACCOUNT_NAME={MY_STG_ACCOUNT_NAME} \
  -e 'AZURE_STORAGE_ACCOUNT_KEY={MY_STG_ACCOUNT_KEY}' \
  kafka-delta-ingest:v0.1.0 \
  ingest web_requests adls2://{MY_STG_ACCOUNT_NAME}/{MY_FILESYSTEM}/web_requests \
  --allowed_latency 5 \
  --kafka {MY_EH_NAMESPACE_NAME}.servicebus.windows.net:9093 \
  --Kafka security.protocol=SASL_SSL \
  --Kafka sasl.mechanism=PLAIN \
  --Kafka 'sasl.username=$ConnectionString' \
  --Kafka 'sasl.password=Endpoint=sb://{MY_EH_NAMESPACE_NAME}.servicebus.windows.net/;SharedAccessKeyName={MY_EH_KEY_NAME};SharedAccessKey={MY_EH_KEY}' \
...
** Replace {MY_EH_NAMESPACE_NAME} with just the namespace name, not the FQDN, of your Azure Event Hubs Namespace.
** Replace {MY_EH_KEY_NAME} and {MY_EH_KEY} with suitable values from your Azure Event Hubs Namespace.
...
* The Delta table must have the `_delta_log` directory and the first Delta transaction containing the schema.
